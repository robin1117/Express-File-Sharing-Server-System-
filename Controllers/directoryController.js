...
export const servingDirectory = async (req, res) => {
    ...
    let directories = await directoryModel.find({ parentDirId: new ObjectId(id) }).lean()
...
async function recursiveDeletionDirectory(id, req) {
    ...
    let directCollection = await directoryModel.find({ parentDirId: new ObjectId(dirId) }).lean()
    // let fileCollection = await db.collection('fileDB').find({ parentId: new ObjectId(dirId) }).toArray()
    let fileCollection = await fileModel.find({ parentId: new ObjectId(dirId) }).lean()
